# Vanilla Policy Gradient with 1 output for Pong
import gym
...
def pg_alg(args):
    ...
    state_shape = (80, 80, 1)
    action_size = 1
    model = VPG2Net(state_shape=state_shape, action_size=action_size, build=args.build)
    ...
    probs = np.zeros((args.max_episode_length, action_size), dtype=np.int32)
    ...
    with tqdm(desc='Episode', total=args.episodes, unit=' episodes') as pbar:
        ...
        for episode in range(args.episodes):
            ...
            for idx in range(args.max_episode_length):
                ...
                action = 1 if np.random.uniform(0, 1, size=None) < policy else 0
            ...
            feed_dict = {
                model.state: states[:eps_len,:],                 
                model.action: actions[:eps_len, np.newaxis], 
                model.advantage: rewards[:eps_len], 
                model.lr: args.learning_rate,
                model.policy_coef: args.coeff_p,
                model.entropy_coef: args.coeff_h
            }
            run_result = sess.run([model.loss, model.mean_entropy, model.entropy, model.policy, model.train_step], feed_dict=feed_dict)
            loss = run_result[0]
            ...
            savefig(fig, axes, 
            	...
            	fname='./save_graph/vpg2_pong_tf.png')
...
def play(args):
    ...
    state_shape = (80, 80, 1)
    action_size = 1
    model = VPG2Net(state_shape=state_shape, action_size=action_size, build=False)
